...
public interface UserProfileService {
     ...
     * 规则：
     * 1. 格式校验：9位纯数字
     * 2. 唯一性校验：globox_no 全局唯一
     * 3. 冷却期校验：距上次修改需满足冷却时间（默认0天）
     * 4. 首次设置时写入 globox_no、last_globox_no_changed_at
     *
     * @param userId  用户ID
     * @param request 设置球盒号请求
     * @return 设置结果（包含 globoxNo 和 cooldownUntil）
     */
    R<SetGloboxNoResultVo> setGloboxNo(Long userId, SetGloboxNoRequest request);

    /**
     * 按球盒号搜索用户
     *
     * 搜索规则：
     * 1. 精确匹配优先（globox_no = input）
     * 2. 前缀匹配其次（globox_no LIKE 'input%'）
     * 3. 支持分页
     *
     * @param keyword  搜索关键词
     * @param page     页码（从1开始）
     ...
     * @return 搜索结果列表
     */
    R<UserSearchResultVo> searchUsersByGloboxNo(String keyword, Integer page, Integer pageSize);
}
