...
public interface RallyParticipantMapper extends BaseMapper<RallyParticipant> {
    ...
    @Select("SELECT * FROM rally_participant WHERE participant_id = #{userId} AND is_voluntarily_cancel = #{status} ORDER BY joined_at DESC LIMIT #{offset}, #{pageSize}")
    List<RallyParticipant> getRallyApplicationByParticipantIdAndStatus(@Param("userId") Long userId,
                                                                       @Param("status") Integer status,
                                                                       @Param("offset") int offset,
                                                                       @Param("pageSize") int pageSize);

    @Select("SELECT COUNT(*) FROM rally_participant WHERE participant_id = #{userId} AND is_voluntarily_cancel = #{status}")
    int countByParticipantIdAndStatus(@Param("userId") Long userId, @Param("status") Integer status);
