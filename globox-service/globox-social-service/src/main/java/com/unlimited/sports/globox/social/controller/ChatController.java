...
import com.unlimited.sports.globox.model.social.entity.MessageResult;
...
public class ChatController {
    ...
    public R<PaginationResult<MessageEntity>> getMessageList(
            @PathVariable Long conversationId,
            @RequestHeader(RequestHeaderConstants.HEADER_USER_ID) Long userId,
            @RequestParam(defaultValue = "1") Integer page,
        ...
        try {
            ...
            if (conversation == null) {
                return R.<PaginationResult<MessageEntity>>error().message("会话不存在");
            }

            if (!conversation.getSenderUserId().equals(userId) && !conversation.getReceiveUserId().equals(userId)) {
                return R.<PaginationResult<MessageEntity>>error().message("无权访问此会话");
            }

            PaginationResult<MessageEntity> messageListByConversation = messageService.getMessageListByConversation(conversationId, page, pageSize);
