...
public class MerchantDubboServiceImpl implements MerchantDubboService {
    ...
    public PricingResultDto quoteVenue(PricingRequestDto dto) {
        try {
            ...
            // 计算槽位价格
            List<SlotQuote> slotQuotes = venuePriceService.calculateSlotQuotes(
                    templates,
                    venueId,
                    venue.getName(),
                    dto.getBookingDate(),
            ...
            if (slotQuotes.isEmpty()) {
                log.warn("无法计算任何槽位的价格");
                return buildErrorResult("无法计算槽位价格");
            }

            BigDecimal totalBasePrice = slotQuotes.stream()
                    .map(SlotQuote::getUnitPrice)
