...
public class CoachServiceImpl implements ICoachService {
    ...
    public PaginationResult<CoachItemVo> searchCoaches(GetCoachListDto dto) {
        ...
        if (dto.getTeachingYearsFilter() != null) {
            switch (dto.getTeachingYearsFilter()) {
                case 1: // 小于3年
                    maxYears = 2;
                    break;
                case 2: // 3-5年
    ...
    public CoachDetailVo getCoachDetail(Long coachUserId, Double latitude, Double longitude) {
        ...
        List<CoachCourseType> services = coachCourseTypeMapper.selectList(
                new LambdaQueryWrapper<CoachCourseType>()
                        ...
                        .orderByAsc(CoachCourseType::getCoachServiceType)
        );

        // 转换课程服务为VO
        List<CoachServiceVo> serviceVos = services.stream()
                .map(service -> {
                    CoachServiceTypeEnum typeEnum = CoachServiceTypeEnum.fromValue(service.getCoachServiceType());
                    return CoachServiceVo.builder()
                            .serviceId(service.getCoachServicesId())
                            .serviceName(service.getCoachServiceName())
                            .serviceType(service.getCoachServiceType())
