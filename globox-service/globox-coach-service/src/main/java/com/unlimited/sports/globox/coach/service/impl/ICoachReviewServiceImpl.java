...
@Slf4j
@Service
public class ICoachReviewServiceImpl implements ICoachReviewService {
    ...
    public void postReview(PostCoachReviewDto dto) {
        ...
        // 处理图片URL列表
        String imageUrls = (dto.getReviewImages() != null && !dto.getReviewImages().isEmpty())
                ? String.join(";", dto.getReviewImages())
                : null;

        // 处理标签列表
        String tags = (dto.getReviewTags() != null && !dto.getReviewTags().isEmpty())
                ? String.join(",", dto.getReviewTags())
                : null;
    ...
    private CoachReviewVo buildReviewVo(CoachReviews review,
        ...
        // 解析图片URL列表
        List<String> imageUrls = (review.getReviewImages() != null && !review.getReviewImages().isEmpty())
                ? Arrays.asList(review.getReviewImages().split(";"))
                : Collections.emptyList();

        // 解析标签列表
        List<String> tags = (review.getReviewTags() != null && !review.getReviewTags().isEmpty())
                ? Arrays.asList(review.getReviewTags().split(","))
                : Collections.emptyList();
        ...
        if (review.getIsAnonymous() != null && review.getIsAnonymous() == 1) {
            ...
            // TODO: 从用户扩展信息中获取NTRP水平
            // userNtrpLevel = userInfo.getNtrpLevel();
            // ntrpLevelDesc = NtrpLevelEnum.getDescription(userNtrpLevel);
        }
