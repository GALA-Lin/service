...
public class UserRefundConsumer {
    ...
    public void onMessage(
            ...
            Channel channel,
            Message amqpMessage){
        log.info("订单已取消但支付成功，自动退款：{}", jsonUtils.objectToJson(message));
        UserRefundRequestDto requestDto = new UserRefundRequestDto();
        BeanUtils.copyProperties(message, requestDto);

        ResultCode resultCode = paymentsService.refundAction(requestDto);

        if (ApplicationCode.SUCCESS.equals(resultCode)) {
